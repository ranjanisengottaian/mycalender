...
const DropdownMenu = ({
  ...
  const dropdownWidth = isYearDropdown ? 'w-32' : 'w-48'; // Reduced width for year dropdown
  ...
  return (
    <div className="relative inline-block mr-5">
      ...
      {/* Dropdown Panel */}
      {isActive && (
        <div className={`absolute left-0 mt-2 ${dropdownWidth} bg-white border border-gray-200 rounded-xl shadow-lg z-20 overflow-y-auto max-h-64 animate-fade-in`}>
          {options.map((item, index) => {
            const value = type === 'month' ? index : item;
            const label = type === 'month' ? item : item;
